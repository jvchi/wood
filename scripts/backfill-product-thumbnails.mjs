...
const THUMB_HORIZON_INSET = Math.round(THUMB_WIDTH * 0.08)
...
async function main() {
  ...
  const pending = rows.filter(row => row.url && !row.thumbnail_url)
  console.log(`Found ${rows.length} product image rows; ${pending.length} need thumbnails.`)
  ...
  for (const row of pending) {
    ...
    try {
      ...
      const availableHeight = THUMB_WIDTH - THUMB_PADDING - THUMB_HORIZON_INSET
      ...
      const top = THUMB_WIDTH - THUMB_HORIZON_INSET - resizedHeight
