...
void main() {
  ...
  // sample the source at this cell's centre. Outside the source → transparent.
  ...
  // transparent paper — only ink (glyph marks) is drawn
  gl_FragColor = vec4(0.0);
  for (int i = 0; i < 4; i++) {
    if (l >= bandLo[i] && l <= bandHi[i]) {
      ...
      // k → 1 paper (transparent), k → 0 solid ink
      float alpha = 1.0 - k;
